...
app.post('/api/calculate', 
  ...
  [
    body('debt')
      .isFloat({ gt: 0 }).withMessage('Debt must be a number greater than 0.')
      .toFloat(),
    body('income')
      .isFloat({ gt: 0 }).withMessage('Income must be a number greater than 0.')
      .toFloat(),
    body('growth')
      .isFloat({ min: 0, max: 100 }).withMessage('Growth must be a number between 0 and 100.')
      .toFloat()
  ], 
  async (req, res) => {
    ...
    try {
      ...
      while (totalRepayment < debt && years < 30) { // Limit to 30 years to prevent infinite loops
        // Find the appropriate repayment rate based on current income
        const repaymentRate = REPAYMENT_BANDS.find(band => currentIncome >= band.min && currentIncome <= band.max)?.rate || 0.0;
